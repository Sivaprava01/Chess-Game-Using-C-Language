        ...
        case 'K': {
            ...
            if (!piece.hasMoved && fromRow == toRow && absCol == 2 && !isKingInCheck(piece.color)) {
                ...
                if (rook.type != 'R' || rook.color != piece.color || rook.hasMoved) {
                    printf("Castling invalid: Rook at %d,%d invalid or moved\n", fromRow, rookCol);
                    return 0;
                }
                int step = (toCol > fromCol) ? 1 : -1;
                ...
                for (int c = fromCol + step; c != rookCol; c += step) {
                    if (board[fromRow][c].type != 0) {
                        printf("Castling invalid: Path blocked at %d,%d\n", fromRow, c);
                        return 0;
                    }
                }
                ...
                for (int c = fromCol; c != toCol + step; c += step) {
                    ...
                    if (isKingInCheck(piece.color)) {
                        ...
                        printf("Castling invalid: King in check at %d,%d\n", fromRow, c);
                        return 0;
                    }
                    board[fromRow][c] = temp;
                    board[fromRow][fromCol] = piece;
                ...
                *isCastling = 1;
                printf("Castling valid: %c from %d,%d to %d,%d\n", piece.color, fromRow, fromCol, toRow, toCol);
                return 1;
            }
            printf("Castling invalid: King moved or wrong distance\n");
            return 0;
        }
    }
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN) {
                ...
                if (selectedRow == -1) {
                    ...
                    if (isMoveValid(selectedPiece, selectedRow, selectedCol, row, col, &isCastling, &isEnPassant)) {
                        printf("Attempting move: %c %c from %d,%d to %d,%d (castling: %d)\n",
                               selectedPiece.color, selectedPiece.type, selectedRow, selectedCol, row, col, isCastling);
                        ...
                        if (isKingInCheck(currentTurn)) {
                            printf("Move invalid: Puts own king in check\n");
                            // Revert move
                        ...
                        printf("Move invalid: %c %c from %d,%d to %d,%d\n",
                               selectedPiece.color, selectedPiece.type, selectedRow, selectedCol, row, col);
                        selectedRow = -1;
                        selectedCol = -1;
                    }
                }
